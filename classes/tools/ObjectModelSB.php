...
class ObjectModelSB extends ObjectModel
{
    /**
     * @var int
     */
    ...
    public function getImage()
    {
        if (static::$lang_image) {
            if (!is_null($this->id_lang)) {
                ...
                $images = array();
                foreach (ToolsModuleSB::getLanguages(!defined('_PS_ADMIN_DIR_')) as $l) {
    ...
    public function uploadImage($tmp_name, $id_lang = null)
    {
        ...
        if ($tmp_name) {
            ...
            if (ToolsModuleSB::checkImage($tmp_name)) {
                $width = null;
                $height = null;
                ...
                $file_path = $this->getFullPathImage($id_lang);
                if (file_exists($file_path)) {
                    unlink($file_path);
                }

                ImageManager::resize($tmp_name, $file_path, $width, $height);
            }
            return true;
        }
        return false;
    ...
    {
        $dir = _PS_MODULE_DIR_.ToolsModuleSB::getModNameForPath(__FILE__).'/views/img/'
    ...
    {
        return _MODULE_DIR_.ToolsModuleSB::getModNameForPath(__FILE__).'/views/img/'
            .Tools::strtolower($this->getClassName()).'/'
            .(int)$this->id.(!is_null($id_lang) ? '_'.$id_lang : '').'.jpg';
    }
